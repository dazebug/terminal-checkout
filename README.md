...
PR 페이지가 아니면 `{main}`의 감지 단계가 없어 리포별 오버라이드 또는 글로벌 기본값이 그대로 쓰입니다 — 기본 브랜치가 `master`인 리포에서 이슈·저장소 버튼에 `{main}`을 쓰려면 오버라이드를 등록하세요 (등록하지 않으면 `git checkout main`이 `pathspec 'main' did not match`로 끊깁니다).
