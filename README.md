...
- **Claude Code hand-off** — if your command runs `claude`, up to 5 scheduled inputs are handed over for you. `!` lines are typed into claude's shell mode so they really run as commands, with a run of them merged into one line; a list that is nothing but plain text is handed over as claude's opening message instead, with no typing at all.
...
**A list that is only plain text becomes the opening message instead.** Plain text is just a message, so it is appended to your command as claude's first argument and the session starts with it already in — no typing, no screen reading, no waiting for claude to boot. The rules for that append:
