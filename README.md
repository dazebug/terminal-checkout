...
- The merged prompt is handed to **`command claude`**, not to `claude`. `command` is POSIX for "skip functions and aliases, run the executable", so a wrapper cannot receive the merged text and do something else with it — structure rather than detection. The cost is deliberate: if you wrap `claude` on purpose, **the merge path bypasses your wrapper** (buttons with no claude input, and the typed route, still use it, and a wrapper under any other name is unaffected).
...
- The merge is also skipped unless your login shell is POSIX-family (`sh`, `bash`, `zsh`, `dash`, `ksh`…). csh and tcsh have no `$( )`, and there the appended text is a parse error that would take the whole command line down with it.
