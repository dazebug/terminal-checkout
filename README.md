...
- **Claude Code hand-off** — if your command runs `claude`, up to 5 scheduled inputs are handed over for you. When they can all be merged they ride along as claude's opening message — `!` lines run in the shell first so their output is already in context. If any one of them can't be merged (a slash command, say), all of them are typed into the session instead, once claude is actually ready.
...
- **Minimal permissions by design** — Chrome itself gets no terminal control. Only the app holds a single "Terminal Checkout → iTerm2" Automation permission; WezTerm needs no TCC permission, and Warp needs the Accessibility permission only for buttons whose claude inputs have to be typed — none of the shipped presets do.
...
- Merging inputs into claude's opening message happens in the app, before the terminal branch, so it works the same on all three terminals. The merged text is assembled by a short throwaway script the app writes to a temp file; the terminal only ever sees its path, and the script deletes itself as claude starts. A request either merges everything or types everything — never both in one session.
- Warp has one extra piece: it has no API for sending text to a pane, so a button whose claude inputs **can't** be merged first launches a small injection helper (`terminal-checkout-warp-helper`, shipped inside the app bundle) in the new tab, and the app hands the inputs to it. The helper writes only into that tab's tty and exits on its own when delivery finishes or the tab closes. Confirming that claude received the input requires reading the screen — that's why typed claude input on Warp needs the Accessibility permission. When everything merges, no helper is launched and no permission is involved.
...
3. **iTerm2 control permission** (shown only when iTerm2 is selected and not yet granted) — click [Request iTerm2 Permission] and allow the prompt. The permission goes to this app only; WezTerm and Warp need none.
   - **Warp claude input** (shown only when Warp is selected and not granted) — allow the Accessibility permission. It's used to confirm on the Warp screen that claude received input that had to be **typed** into the session. Without it such a button is **refused outright**: no tab opens, and the button shows ❌ rather than running the command with the input missing. Keep the tab visible during delivery. Input that merges into claude's opening message — which is all of it for the shipped presets — never goes through this path and needs no permission.
...
If the command runs `claude`, the options page lets you schedule up to 5 inputs per button — e.g. `!gh pr diff {number}` followed by `Summarize the risky parts`. They reach claude by one of two routes, chosen automatically: **either all of them are merged into claude's opening message, or all of them are typed into the session.** Never a mix — one session using both meant racing claude's own startup, and losing that race destroys input silently.

**Merged into the opening message (the fast route).** The app folds the inputs into the single prompt claude starts with. This route needs no permissions, no screen reading, and no waiting for claude to boot.
...
- A **boundary** anywhere in the list sends *every* input down the typed route: ① a line the input box reads specially — a **slash command** or a **`#` memory line** (both only mean that when they are the whole message typed into the box, so merging would silently turn them into ordinary text), and ② a **`!` command that follows a plain-text input** (its output was meant as context for the instruction *after* it, so hoisting it into the opening message would change what the earlier instruction sees).
...
- The merged prompt is handed to **`command claude`**, not to `claude`. `command` is POSIX for "skip functions and aliases, run the executable", so a wrapper cannot receive the merged text and do something else with it — structure rather than detection. It does **not** skip shell builtins, which is why a command that loads one (`zmodload`, `enable`) stops the merge instead.
- Because of that, **merging needs a `claude` executable to exist**. The app asks your login shell at startup — in a child shell, so a function or an alias of yours does not hide the file behind it, and it checks the file is actually runnable. Two outcomes: if a file exists, the merge runs it and **bypasses any wrapper you put around that name** (name the wrapper something else to keep it); if `claude` is *only* a function or an alias (`alias claude='npx @anthropic-ai/claude-code'`), `command claude` would fail, so those setups take the typed route instead — which works with a wrapper. The setup window says so when that is the case.
- The answer comes from your **login shell**, and it can be wrong in one direction: a PATH entry that is relative, or a `claude` that only exists in a shell the app cannot ask, reads as "no executable" and simply costs you the merge. On Warp without the Accessibility permission that costs more than speed — everything then has to be typed, and a button that cannot be delivered is refused.
- The merge only happens when the rendered command is a plain chain (`&&`, `||`, `;`, `|`, groups, subshells) whose **last command is a bare `claude`** — no flags, not on the receiving end of a pipe, no redirect, nothing after it. Flags are out because some of them (`--resume`) swallow the prompt as their own value; the rest change what "the last command" means. A comment, a heredoc, a line continuation, a newline, a background `&`, a backtick or a `$(…)` stops the merge too.
- Beyond that, **every word of the command has to be one that would be safe as a command name**: nothing that can rebind a name in that shell (`function`, `alias`, `eval`, `source`/`.`, `hash`, `trap`, `export`, an assignment like `PATH=…` or `functions[claude]=…`, a compound keyword such as `if`/`for`/`while`/`case`) and nothing quoted or expanded, which we cannot read. Every word rather than the first, because a redirection (`> /dev/null eval …`) or a shell modifier (`noglob`, `nocorrect`) moves where the command name actually is. The price is over-folding: `git add . && claude` and anything with a quoted argument fall back to the typed route, which is what they did before this route existed.
- The merge is also skipped unless your login shell is POSIX-family (`sh`, `bash`, `zsh`, `dash`, `ksh`…). csh and tcsh have no `$( )`, and there the appended text is a parse error that would take the whole command line down with it. The login shell is a **proxy**: the shell your terminal opens a tab with is chosen by the terminal and can differ, and the app has no way to ask before the tab exists. If yours differ and the tab's shell is csh-family, the merged command fails in the pane with `Illegal variable name.` — remove the claude inputs from that button, or don't end the command in a bare `claude`.
- What the app still cannot see, because it needs your shell and filesystem at run time: a **`PATH` that resolves `claude` to a different program** (set in your rc, or by a command that writes one — `cp x ~/bin/claude && claude`), and a **`command` function or alias in your rc**, which would capture the invocation above. In the first case a different program receives the prompt as its argument; in the second, a wrapper does.
- An input containing a NUL byte is rejected outright rather than delivered altered — a command line can't carry it and neither can a tty.

**Typed into the session (the fallback route).** When anything blocks the merge, the command is left exactly as you wrote it and *all* the inputs are typed in order, only after confirming the new tab's foreground process has become claude — the delivery gates are designed to fail closed, dropping input rather than typing into a shell; if claude doesn't appear within 2 minutes, the inputs are dropped and the app log says so. Each input is submitted only after it's confirmed as actually typed on screen, so delivery holds while claude's trust prompt for a first-time folder is up — accept within 15 seconds and it continues; take longer and delivery is abandoned from that input on. Two different questions get two different answers here. **Did claude receive it** is never established — nothing outside the TUI can, so the log says "sent", not "delivered". **Is the input box empty before the next input is typed** is a safety question, and it is not guessed: the box is cleared with Ctrl+U immediately before every input and once more when delivery ends, unless the app can see that nothing of ours is in there. That is what keeps a leftover from being appended to the next input and submitted as one mangled line, and it does not depend on the screen being readable. The look after the return key only decides whether to **stop early** — if it can still see that input sitting in the box, there is no point typing the next one — and it stays silent whenever it cannot identify our text (a screen it can't read, our text also drawn in a hint line below the box, a context meter ticking underneath). The cost is deliberate and worth stating by size: delivering N inputs sends **at least N Ctrl+U** into the session — one as part of each input's check, plus one more when delivery ends unless the app could see that nothing of ours is in the box — where earlier versions sent none on a clean run. A draft you start typing during delivery is erased rather than mixed into ours. The alternative is our `!` line waiting in the box for your next Enter. One consequence to know about: if claude never acted on the return key for one input, the next input's clear wipes it — the app still logs that one as *sent*, because it cannot tell. That is a lost input, not a mangled or duplicated one. An input whose return key has gone out is never retyped either way: the key may have landed even when the call reported failure, and retyping is the one move that could run a `!` command twice.

Known limits:

- Inputs are single-line only.
- A button whose inputs have to be typed is **refused before anything opens** when the app can already tell it couldn't deliver them: on Warp without the Accessibility permission or without the bundled injection helper, and on WezTerm when no WezTerm window is running (a fresh process gives no addressable pane). The button shows ❌ and no tab is created — better than a claude session sitting there with none of its context. Buttons whose inputs all merge, and buttons with no claude input, are unaffected.
...
**claude input isn't delivered on Warp** — This only affects input that had to be typed into the session; if it merged into the opening message, permissions play no part. If the button showed ❌ and no tab opened, the app knew up front it couldn't deliver: grant **Accessibility** in the setup window (the window comes forward on its own to show you) or reinstall to restore the bundled helper. If the tab did open and only the input is missing: were you looking at that tab until delivery finished? Switching away makes the app wait (it resumes when you return). Otherwise the reason is in `log show --predicate 'subsystem == "com.dazebug.terminal-checkout"' --last 15m --info`.

**A claude input didn't merge into the opening message** — Either the input list has a boundary in it (a slash command, a `#` memory line, or a `!` after plain text) or the command can't take the merged prompt: it must end in a bare `claude`, with no trailing flag (`claude --resume`), redirect, pipe or comment, every word has to be readable and safe as a command name (`git add .`, `-m 'msg'`, `export …` and `PATH=…` all stop it), your login shell has to be POSIX-family, and `claude` has to be a real executable rather than a function or an alias. Everything then goes to the typed route, which is the old behaviour, not a failure — but on Warp it means the Accessibility permission comes back into play.
