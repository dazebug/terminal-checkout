...
3. **Terminal** — choose iTerm2, WezTerm, Warp, or cmux
   - **cmux socket control** (shown only when cmux is selected) — the card shows not installed, not running, denied, or reachable and offers [Allow Automation] when needed. Automation means every process run by the same macOS user can control cmux. Clicking it backs up `~/.config/cmux/cmux.json` as a timestamped `.bak` and records `automation.socketControlMode=automation`; installation, app launch, and refresh only read the live status.
...
**cmux socket access is denied** — In the setup window's cmux card, click [Allow Automation], or edit `~/.config/cmux/cmux.json` and set `automation.socketControlMode` to `automation`.

**cmux is not running** — [Run in Terminal] starts cmux without arguments; after its socket appears, use [Check Again]. The [Allow Automation] action only records the approved setting and then checks whether cmux picked it up.
