...
- Because of that, **merging needs `claude` to be a real executable**. If you install it as a function or an alias (`alias claude='npx @anthropic-ai/claude-code'`), `command claude` would fail with "command not found" — so the app checks, at startup, whether the name resolves to a file and sends those setups down the typed route instead, which works with a wrapper. Buttons with no claude input are never touched either way. If you wrap an executable `claude` on purpose, the merge path **bypasses your wrapper**; naming it something else keeps it.
