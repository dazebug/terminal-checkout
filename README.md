...
- A **boundary** anywhere in the list sends *every* input down the typed route: ① a **slash command** (it only acts as a command when it's the whole message, so merging would silently turn it into ordinary text), and ② a **`!` command that follows a plain-text input** (its output was meant as context for the instruction *after* it, so hoisting it into the opening message would change what the earlier instruction sees).
...
- The merge only happens when the rendered command is a plain chain (`&&`, `||`, `;`, `|`, groups, subshells) whose **last command is a bare `claude`** — no flags, not on the receiving end of a pipe, no redirect, nothing after it. A comment, a heredoc, a line continuation, a newline, a background `&`, a backtick or a `$(…)` also stops the merge, and so does any **command position** holding something that could make `claude` mean something else: `function`, `claude() {…}`, `alias`, `eval`, `source`/`.`, `hash`, `trap`, `export`, a `PATH=…` assignment, or a compound command (`if`, `for`, `while`, `case`). Flags are out because some of them (`--resume`) swallow the prompt as their own value; the rest are out because they change what "the last command" means. Anything else falls back entirely to the typed route.
- Two things the app cannot see, because both need to know your shell and your filesystem at run time: a `claude` **function or alias defined in your shell's rc**, and a command that *installs* a `claude` earlier on your `PATH` (`cp x ~/bin/claude && claude`). In either case the merged prompt arrives as that wrapper's first argument instead of claude's. If you wrap `claude`, expect that.
...
**A claude input didn't merge into the opening message** — Either the input list has a boundary in it (any slash command, or a `!` after plain text) or the command can't take the merged prompt: it must end in a bare `claude`, with no trailing flag (`claude --resume`), redirect, pipe or comment, and nothing in command position that could rebind the name (`function`, `alias`, `eval`, `source`, `export`, `PATH=…`, `if`…). Everything then goes to the typed route, which is the old behaviour, not a failure — but on Warp it means the Accessibility permission comes back into play.
