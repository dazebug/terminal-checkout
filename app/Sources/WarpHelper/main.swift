...
//  5. **Removing what the app put there**, which is the same move in reverse and is not covered by 4: taking an address back occupies the advertised name, and a same-uid process that deletes it before a delayed helper's `link` turns a refused claim into a successful one — a helper that outlives the app. The same goes for the pin the app links from (`warpHelperPinPath`): deleting it before termination leaves the app with nothing to link, which is the `.failed` state, and that state is at least reported rather than mistaken for a dismissal. All of this is inside the declared boundary and is not prevented; what changes is that residual 1's "narrowed at both ends" is a statement about accidents, not about this (round 19 review).
