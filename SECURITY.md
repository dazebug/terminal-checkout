...
- **Same-uid processes are trusted.** The app's unix socket (mode 0600 + peer verification), cmux's automation socket when socket control mode is `automation`, and, on Warp, the injection helper's socket intentionally accept any process running as the same user. Same-uid access is not considered a boundary violation; the helper's exposure is narrowed by lifetime instead — it is killed the moment delivery ends.
