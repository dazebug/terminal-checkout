...
- claude 입력(`claude_inputs`): command가 띄운 claude 세션에 순서대로 타이핑할 자유 텍스트(버튼당 최대 5개, 같은 변수 치환). 앱이 세션 tty의 포그라운드 프로세스가 claude/node/bun임을 `ps`로 확인하고, 이어서 tty가 raw mode(`stty`의 `-icanon`)인지 확인한 뒤에만 전송한다(`Core/ClaudeInjector.swift`) — 포그라운드 확인을 없애면 셸에 Enter 포함 오입력되어 즉시 실행되므로 우회 금지. raw mode 확인은 셸이 claude를 exec한 직후의 canonical 구간(exec 후 0.1∼1초)을 배제한다: 그 구간에 타이핑하면 claude가 아니라 커널이 에코하고, `screenShowsInput`이 그 에코를 반영으로 오판해 CR을 일찍 보내면 claude가 화면을 다시 그릴 때 CR만 유실돼 첫 입력이 제출되지 않고 입력창에 매달린다(실측). raw mode에서는 커널 에코가 꺼지므로 게이트 통과 후 화면에 보이는 텍스트는 claude가 그린 것이고, 그래서 반영 확인이 비로소 수신을 뜻한다. `stty`를 읽지 못하면 판정 불가로 보고 `ps`만으로 진행한다 — 전달을 통째로 포기하는 것보다 낫기 때문. 또 처음 준비된 claude의 **PID를 고정해** 이후 입력이 같은 세션에만 가도록 한다: 이름과 raw mode만 보면 원래 세션이 죽은 뒤 같은 tty에 새로 뜬 claude를 같은 세션으로 오인해 남은 입력을 흘리고, `!…` 셸 모드 입력이면 의도치 않은 명령까지 실행된다. `ps -o pid=`의 pid는 우측 정렬이고 comm 앞에도 칸 맞춤 공백이 붙으므로 파싱에서 공백을 반드시 떼야 한다(실측). 각 입력은 [개행 없는 타이핑 → 화면 반영 확인 → CR(\r) 제출] 순서다 — LF(\n)는 제출로 인식되지 않고(실측), 게이트를 통과한 뒤에도 claude TUI가 입력을 아직 그리지 못하는 순간이 있어 반영을 확인하기 전에는 CR을 보내지 않는다. 이 시퀀스를 "텍스트+\n 한 번에 전송"으로 단순화하면 안 된다. 반영 확인은 공백을 지우고 비교한다(`screenShowsInput`) — claude는 shell mode 입력을 `! gh …`로 공백을 끼워 그리고 긴 입력은 줄바꿈해서, 통짜 비교로는 영영 확인에 실패해 입력이 매달린다(실측). 전달 감시는 execQueue 밖 백그라운드에서 돈다(Chrome 응답을 막지 않기 위해). WezTerm fallback(`wezterm start`) 경로는 pane을 특정할 수 없어 전달 생략
...
- 버튼 순서 변경(드래그·↑↓)과 복제는 옵션 페이지 전용이지만, 배열 조작은 순수 함수 `moveButton`·`duplicateButton`으로 `defaults.js`에 있다(`tests/buttons.test.js`가 검증). 드래그는 손잡이(`.drag-handle`)를 누르고 있는 동안에만 카드에 `draggable`을 건다 — 카드에 상시로 걸면 안쪽 입력에서 텍스트를 끌어 고를 수 없다. 드롭하면 곧바로 다시 그려 원래 카드가 문서에서 빠지므로 뒷정리는 `dragend`가 아니라 `drop`에서 끝낸다. 브라우저 자동화로 검증할 때 CDP 합성 입력은 `dragstart`∼`dragend`까지만 재현하고 네이티브 `drop`을 완결하지 못하므로(실측), 드롭 이후 로직은 합성 `DragEvent`로 확인한다
