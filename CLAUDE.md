...
- 화면 반영 확인은 **타이핑 직전 화면과 비교해 프로브가 한 번 더 보일 때만** 통과다(`screenReflectsNewInput`). "화면에 있나"만 보면 Warp에서 접근성으로 읽히는 다른 pane의 텍스트에 걸려, 우리 입력은 제출되지 않은 채 그 pane에서 사용자가 치던 것이 제출된다. 스냅샷을 못 찍었으면 확인 실패로 다룬다
