...
- command template 시스템: `{repo}`, `{owner}`, `{number}`, `{branch}`, `{main}`, `{branch_underbar}` 변수를 앱(`Core/CommandRenderer.swift`)에서 치환. 값 검증은 영숫자·`-_./` 화이트리스트 문자 검사 (파이썬 시절 정규식 `$`가 끝 개행을 허용하던 구멍까지 막은 것 — 정규식으로 되돌리지 말 것). 이슈 페이지는 head 브랜치가 없어 `{branch}` 계열을 주지 않으므로, 템플릿이 쓰면 앱이 not provided로 거절한다
