...
- 화면 반영 확인을 tty 입력 큐 신호(`FIONREAD`가 0)로 대신해도 되는 자리는 **화면을 아예 읽을 수 없을 때와 재타이핑을 다 쓴 뒤**뿐이다. 큐가 비었다는 것은 claude가 `read()`했다는 뜻일 뿐 입력창에 그렸다는 뜻이 아니다 — 화면은 읽히는데 입력만 안 보이는 중간 상태에서 이 신호로 제출하면, claude가 뜬 직후 아직 그리지 못해 버린 입력이 "전달됨"으로 기록되고 빈 줄만 제출된다(Warp 실측)
