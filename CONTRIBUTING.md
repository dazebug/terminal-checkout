...
git config core.hooksPath .githooks   # keeps the AGENTS.md symlink in sync
```
...
- Read [`CLAUDE.md`](CLAUDE.md) first. It records the architecture constraints and empirically measured pitfalls this project depends on — in particular the TCC responsible-process split and the claude-input injection gates, which must not be weakened.
