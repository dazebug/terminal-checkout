...
**claude input — typed route (everything except a single plain-text input, so: every shipped preset)**
...
The payload above is typed (its input is a `!` line), which is the route every shipped preset takes. To exercise the argv route instead, use a single plain-text input (`["summarise the diff"]`) with a command ending in a bare `claude`.
