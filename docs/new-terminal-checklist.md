...
Claude inputs reach the session by two routes and **only the second one is per-terminal.** `ClaudeInputPlan.swift` either merges **every** input into the argv prompt of the `claude` the command starts — before any terminal branch is taken — or merges none and leaves all of them to `ClaudeInjector`; there is no request that uses both. (Mixing them meant racing claude's own startup submission, which clears the input box, and no gate over that race can be built from signals the user's shell cannot forge — a zsh with `set -x` prints the substituted argv, marker and all, before the exec.) So a new terminal inherits the merged route for free and the table above only covers the typed route. Two consequences when adding a terminal: the typed route is exercised by fewer buttons than before (none of the shipped presets reach it, so you have to write a slash-command input by hand to test it at all — see the hands-on list), and a terminal that quotes or rewrites the command it is handed must survive `… claude -- "$(/bin/sh '<path>' || …)"` — the appended substitution is ordinary command text, but it is the first place a new escaping bug will show.
...
- [ ] A command that can't take the append (`… && claude --resume`, `… | claude`, a trailing comment, a heredoc, a multi-line command, or anything in **command position** that can rebind a name — `function`/`alias`/`eval`/`source`/`export`/`PATH=…`/`if`…) → every input goes to the typed route instead, i.e. today's behaviour
- [ ] Each run gets its own `$TMPDIR/tc-prompt-<8 hex>/` at mode 0700. `prompt.sh` inside is gone once the tab's command reaches `claude`; **`context.txt` deliberately stays** — deleting it before `execve` would lose the assembled text if the exec then failed, so an age-based sweep reclaims it instead (6 hours, or 7 days when `handed-to-claude` is present because claude was told to read the file). A run that fails before the terminal opens removes the whole directory immediately
...
- [ ] With a `claude` **function or alias in your rc**, the merge hands it the prompt as `$1`. The app cannot see rc definitions, so this is a documented residual, not a bug to fix in the terminal branch — just confirm it behaves the way that wrapper would with any argument. The same residual, for the same reason, covers a command that **installs** a `claude` earlier on PATH (`cp x ~/bin/claude && claude`): both need runtime knowledge of the shell and the filesystem, which the scanner does not have. What it *does* catch is every rebinding written in the command itself, because rebinding a name requires a builtin or keyword running in that same shell, and that can only sit in command position

**claude input — typed route (used only when nothing merges)**
...
- [ ] **A boundary anywhere sends everything down this route** (e.g. `!gh pr diff {number}` then `/review`): the command is left untouched, claude starts empty, and both inputs are typed. Confirm the command really is unchanged — one merged message plus typed leftovers in the same session is the combination that was removed, because submitting the merged message clears the input box and wipes anything typed before it renders (measured on claude 2.1.238: raw mode at 0.1∼0.19s, message render at 2.06∼3.41s)
- [ ] A submitted input is **still on screen afterwards**. The app only counts an input as delivered once it can see it survived the CR — sending the key is not evidence that a message exists. If this terminal's screen read cannot show a submitted message (transcript not readable, viewport too small), delivery stops after the first input and the log says so
...
The payload above merges (its one input is a `!` line and the command ends in `claude`), so it exercises the merged route. Adding a slash command anywhere — `["/help","!echo ok"]` or `["!echo ok","/help"]` — sends **every** input down the typed route instead, which is how you exercise it.
...
- **What the merged message will be**: read the generated script before running it — `cat "$TMPDIR"/tc-prompt-*/prompt.sh`, or run it by hand to see the exact text claude will receive. The script deletes itself once the tab's command reaches `claude`; the `context.txt` beside it is left for the age-based sweep
