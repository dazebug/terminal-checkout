...
- **The app's verdict**: `/usr/bin/log show --predicate 'subsystem == "com.dazebug.terminal-checkout"' --last 15m --info` — on success it records a delivered-count line for the typed route (the app logs it in Korean today: `입력 N개 중 M개 전달`). A run that went entirely through argv logs nothing there, because nothing was delivered by injection. The absolute path matters because `log` can be shadowed by shell functions/aliases
