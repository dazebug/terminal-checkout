...
- [ ] A command that can't take the append (`… && claude --resume`, `… | claude`, a trailing comment, a heredoc, a multi-line command) → every input goes to the typed route instead, i.e. today's behaviour
...
- [ ] With a `claude` **function or alias in your rc**, the merge hands it the prompt as `$1`. The app cannot see rc definitions, so this is a documented residual, not a bug to fix in the terminal branch — just confirm it behaves the way that wrapper would with any argument
...
- [ ] **Both routes at once** (e.g. `!gh pr diff {number}` then `/review`): nothing is typed until the merged message is on screen, and then the leftover is delivered. Submitting the merged message clears the input box, so typing early loses the leftovers outright — and the readiness gates are far too early to help (measured on claude 2.1.238: raw mode at 0.1∼0.19s, message render at 2.06∼3.41s). The app watches for the `==== tc-… ====` first line and requires it to appear **one more time than before claude started** (a shell with `set -x` echoes the resolved command line first, and existence alone would take that for the render). So **confirm that line really is visible in this terminal's screen read** — if the TUI collapses or hides it, the leftovers get dropped after 2 minutes and the app log says so
