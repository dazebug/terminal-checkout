...
- [ ] With a `claude` **function or alias in your rc**, the merged run **bypasses it**: the append invokes `command claude`, which skips functions and aliases (measured in zsh, bash and dash). Confirm the session starts from the executable and that nothing the wrapper adds is present — that is the accepted cost of not handing user text to an unknown wrapper. Residuals to confirm as *documented*, not as bugs: a PATH that resolves `claude` elsewhere (rc-set, or installed by the command itself — `cp x ~/bin/claude && claude`), and a `command` function or alias in the rc. Both need runtime knowledge of the shell and the filesystem, which no scanner has. The word list in `commandAcceptsAppendedClaudePrompt` is a second layer over that structure, and its completeness is **not** claimed
...
- [ ] **After the CR the screen has to move.** Where the screen belongs to our pane by construction (session/pane id), the app requires the screen to differ from the one the input was seen typed in, with the input still visible — a frozen screen means the CR did nothing, and the text gone means the box was emptied without a message. Both stop delivery. Where the screen cannot be attributed to our pane (Warp), the check is skipped and says nothing: reading someone else's tab as "our input vanished" reported a delivered input as lost and dropped the rest. So for a new terminal, decide which of the two it is (`TerminalSessionHandle.screenNeedsPaneProof`) and check that switching tabs mid-delivery does not fail a delivery that went through
