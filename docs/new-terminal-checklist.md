...
- [ ] `{cd}` with no base directory set — renders as the zoxide jump alone (`{ tc_dir=$(zoxide query --list -- {repo} | …) && cd -- "$tc_dir"; }`). It has to land in the folder named exactly `{repo}` even when a `{repo}-<branch>` worktree was used more recently, and even when the tab opens inside `{repo}` itself; a pane whose shell cannot find `zoxide` dies at the first step with `command not found`
- [ ] `{cd}` with a base directory set and no folder named exactly `{repo}` in zoxide (a cold DB, or no zoxide at all) — the fallback lands in the repository and the chain runs to its end
...
- [ ] Select zero rows and then more than twenty-five rows → clicking a list button sends no batch request, shows the phase marker plus localized title diagnosis, and leaves the twenty-sixth checked until the user changes it
...
- [ ] Select two or more PR rows and run the default PR-list preset → each session lands in its own `../<repo>-pr-<number>` worktree on that PR's head (detached), no session's checkout changes under another, and rerunning a row reuses its worktree with the refreshed head
...
- [ ] A **plain-text-only** button (no `!`, no slash, no `#`) → claude starts with that text already submitted, and nothing is typed. Shipped presets with scheduled inputs are all `!`, so they do **not** take this route any more — they are typed (see below)
- [ ] A shipped preset with `!` inputs (Review PR / Read Issue / issue-list triage) → the commands **run in claude's shell mode**, one merged line, banners between them, in the directory claude started in (check one whose command `cd`s into a worktree). They appear in the session as commands, not as claude deciding to run something
