...
- [ ] **The input box is emptied as part of every input and once when delivery ends** — that, not the screen, is what stops a leftover from being appended to the next input. Count them: one Ctrl+U per input (the attribution experiment below) plus **one at the end of every delivery, always** — a successful delivery is not exempt, because a written CR is not evidence the box is empty. Type into a session by hand while delivery is running: your draft is erased (accepted cost, issue #16), and nothing of ours is left behind for your Enter
- [ ] **Each input is typed twice on purpose**: once to find out whether what appears on screen is really in the input box (type → clear → watch it disappear), then again for the submission. Nothing is submitted between the two, so this cannot duplicate a message — but check the terminal survives the extra round trip, and that a `!` input still enters shell mode when it is typed into the **empty** box the experiment leaves behind
