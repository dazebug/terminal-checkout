...
- [ ] **The input box is emptied before every input and once when delivery ends** — that, not the screen, is what stops a leftover from being appended to the next input. Type into a session by hand while delivery is running: your draft is erased (accepted cost, issue #16), and nothing of ours is ever left behind for your Enter
