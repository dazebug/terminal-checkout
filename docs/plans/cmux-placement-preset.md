...
- 현재: R1 반영 완료 · 항목 1 cleared · 항목 2 cleared · 항목 3 cleared · 항목 4 verified
- 최근 검증자 판정: "항목 3 차단 (a)∼(d) 막힘 확인 — grouped 실행 전 경로가 테스트 11건과 드라이버 실서버 실측으로 고정, 새 표면의 실효 우회 없음. 잔여(비차단): enumeratedSurfaceIDs는 leafItemOrder가 identity라는 현 plan 생성기 계약에 기댄다." (드라이버, R1) · 원문 없음
...
| `app/Sources/Core/CmuxGroupedExecution.swift` · `workspace.list` | 처리됨(항목 3) | target 없음은 D1의 현재-window 범위 계약; response `workspaces[]`, `has_custom_title`, `custom_title`, `index`는 D1의 드라이버 실측 근거 |
| `app/Sources/Core/CmuxGroupedExecution.swift` · `workspace.create` | 처리됨(항목 3) | window target 없음은 U2의 무주소 current-window 계약; params `focus`, `layout`, `operation_id`, optional `title`; response `workspace_id`와 focused-leaf `surface_id`는 issue #68 items 7, 11, 12, 13 |
| `app/Sources/Core/CmuxGroupedExecution.swift` · `pane.list` | 처리됨(항목 3) | target `workspace_id`는 명시하고 response 항목은 `id`/`index`(드라이버 실측 근거: `panes` 응답의 각 항목에 `id` 존재). `pane_id`는 사용하지 않음 |
| `app/Sources/Core/CmuxGroupedExecution.swift` · `surface.list` | 처리됨(항목 3) | target `workspace_id`·`pane_id`는 명시하고 response 항목은 `id`/`index_in_pane`(드라이버 실측 근거: `surfaces` 응답의 각 항목에 `id` 존재). `surface_id`는 사용하지 않음 |
| `app/Sources/Core/CmuxGroupedExecution.swift` · `surface.split` | 처리됨(항목 3) | target `surface_id`·`direction`을 매 호출 명시(U4); response `surface_id`/new pane는 issue #68 item 2; split retry 없음 |
| `app/Sources/Core/CmuxGroupedExecution.swift` · `surface.create` | 처리됨(항목 3) | target `workspace_id`·`pane_id`를 매 호출 명시; response `surface_id`는 issue #68 items 2, 8; create retry 없음 |
| `app/Sources/Core/CmuxGroupedExecution.swift` · `surface.send_text` | 처리됨(항목 3·4) | target `surface_id`와 `text`를 매 호출 명시; `queued` response는 issue #68 item 6; 기존 send gate를 통과한 guarded item만 보내고 retry 없음; `deadlineExceeded`를 각 item 처리 전에 확인해 budget 이후 item은 command를 보내지 않고 `CommandError.badRequest`로 닫음 |
| `app/Sources/Core/TerminalRunner.swift` · `debug.terminals` gate / `cmuxRPC` door | 처리됨(항목 3) | `TerminalRunner.swift:393-414,489-520,567-613` — 모든 grouped RPC가 기존 `cmuxRPC`와 pinned socket context를 공유하고, `debug.terminals`의 `surface_id`·`tty`는 issue #68 item 6 근거지만 tty는 readiness 신호가 아님 |
