...
- 현재: R1 반영 완료 · 항목 1 cleared · 항목 3 cleared · 항목 2 verified
...
| `app/Sources/Core/CmuxGroupedExecution.swift` · `surface.send_text` | 처리됨(항목 3) | target `surface_id`와 `text`를 매 호출 명시; `queued` response는 issue #68 item 6; 기존 send gate를 통과한 guarded item만 보내고 retry 없음 |
