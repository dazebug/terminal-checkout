...
- Evidence conflict to preserve: the driver review says system.tree and system.identify are absent from the methods list and directs the plan to use list RPCs plus CLI tree, while the raw capabilities.json read here contains both strings in its methods array. This plan follows the explicit review decision for the oracle surface and leaves the conflict open rather than treating either source as silently authoritative.
...
- The driver review excludes system.tree and system.identify as RPC oracles, but the raw capabilities.json read for this plan contains both names in its methods array. The plan follows the review's list/current plus CLI-tree decision; the final context update must resolve or preserve this source conflict explicitly.
