...
- 현재: R1 · 마지막 승격 0d575ed · 리뷰 중 없음 · A∼E cleared (네 게이트는 드라이버가 실행함)
...
| D13 | 드라이버 | /tmp 경유 포인터·소켓의 다른 uid 노출 | 이번 범위에서 고치지 않고 기록만 한다 — 상정 행위자는 같은 uid이고, 이 경로는 다른 로컬 계정이 cmux보다 먼저 이름을 선점해야 닿는 (A)류다 | 실측: nightly의 live socket은 /tmp/cmux-nightly.sock, 포인터 후보에 /tmp 사본 포함; sticky bit는 선점을 막지 않는다 | 완화안은 해석된 소켓 타깃의 st_uid를 getuid()와 대조하는 것 — 다중 사용자 Mac을 상정 행위자에 넣게 되면 그때 배정 |
| D14 | 드라이버 | 테스트 심사에서 단일 포인터 resolver 대역이 다중 후보 대역과 중복됨 | `testCmuxResolvedSocketPathRequiresALivePointerTarget`를 삭제하고 단일 원소·live/dead·공백·nil 경계를 `testCmuxSocketPointerCandidatesPreferStateThenTmpAndRequireALiveTarget`에 흡수한다 | `git diff e2e9c43..HEAD -- app/Tests`의 클래스 단위 심사; 다중 후보 resolver는 단일 후보도 같은 `cmuxResolvedSocketPath` 경계를 통과함 | `cmuxSocketPin`, 후보 목록, `Terminal` rawValue, SetupWindow 라디오·pipeline 계약은 서로 다른 경계로 유지 |
...
#### 리뷰 4 — 증분 · 0d575ed · 드라이버

- 실측: 게이트 swift 555(1 skip)·node 222·build·e2e 통과
- 판정: "C·D·E에 차단 없음 — 원 요구 2건 모두 코드·테스트·문서로 닫힘."
- 비차단: R1(지어낸 근거 한 줄) 수정, R2(/tmp 다른 uid 노출) 원장 기록
