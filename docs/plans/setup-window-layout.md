...
- 현재: R0 종료 · 마지막 승격 R0 계획 커밋 · 리뷰 중 없음 · 게이트 그린(드라이버 환경: swift 481 tests / 1 skipped / 0 failures, node 216 pass / 0 fail — 기준 트리 a13fa48)
...
|:--|:--|:--|:--|:--|:--|:--|:--|:--|
| 1 | R1에서 최소 Accessibility provider seam과 false→true 권한 전이 재현 fixture만 추가한다 | R1 재현·test seam | (a) `PermissionChecker.isAccessibilityGranted`가 `accessibilityIsTrusted()`를 직접 호출해 기존 테스트가 전이를 만들지 못함 | `app/Sources/App/PermissionChecker.swift`, `app/Tests/AppTests/SetupWindowLayoutTests.swift` | — | todo | `PermissionChecker.swift:49∼61`; `SetupWindowLayoutTests.swift:203∼214`는 실제 상태를 읽고 lower-bound만 검사 | |
| 2 | R1에서 `settle`을 고정점 반복으로 바꾸고 `layout()` 재진입·pass별 fitting 값을 계측한다 | R1 수렴 계측 | (a) 현재 `settle(_:)`은 `layoutSubtreeIfNeeded()`를 한 번만 호출함 (b) layout 재진입과 pass별 target 변화가 기록되지 않음 | `app/Sources/App/SetupWindowController.swift`, `app/Tests/AppTests/SetupWindowLayoutTests.swift`, `app/Tests/AppTests/SetupWindowRedrawTests.swift` | 1의 권한 전이 입력 | todo | `SetupWindowLayoutTests.swift:56∼58`; `SetupWindowController.swift:47∼56`; 드라이버 baseline `swift test --package-path app` → 481 tests, 1 skipped, 0 failures | |
| 3 | R1에서 `8fccab5`의 선행 계약과 override 밖의 `window.screen`·`NSScreen.main` 선택 분기를 Mac Pro + 외장 2대(가로 1, 세로 1)에서 대조한다 | 선행 수정·화면 선택 | — | `app/Sources/App/SetupWindowController.swift`, `app/Tests/AppTests/SetupWindowLayoutTests.swift`, `app/Tests/AppTests/SetupWindowRedrawTests.swift`, `8fccab5` | 1∼2의 재현 계측 | todo | `git show 8fccab5`; `SetupWindowController.swift:25∼56`; 현재 테스트는 `visibleFrameOverride`만 사용 | |
...
#### 리뷰 1 — 미실시 · 커밋 없음 · 승격 없음 · 리뷰 미실시 · 왕복 0 · 원문 없음
