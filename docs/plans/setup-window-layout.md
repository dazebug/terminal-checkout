...
- 현재: R2 · 마지막 승격 <이 커밋 SHA> · 리뷰 중 없음 · 게이트 그린(481 / 1 skipped / 0 failures)
...
| 2 | R1에서 `settle`을 고정점 반복으로 바꾸고 `layout()` 재진입·pass별 fitting 값을 계측한다 | R1 수렴 계측 | (a) 현재 `settle(_:)`은 `layoutSubtreeIfNeeded()`를 한 번만 호출함 (b) layout 재진입과 pass별 target 변화가 기록되지 않음 | `app/Sources/App/SetupWindowController.swift`, `app/Tests/AppTests/SetupWindowLayoutTests.swift`, `app/Tests/AppTests/SetupWindowRedrawTests.swift` | 1의 권한 전이 입력 | verified | 재실행: 같은 명령 → 고정점 settle(상한 20) 도입 후 기존 6 케이스 전원 통과, 신규 케이스만 red. TRACE: granted pass=1 fitting=747.0 target=747.0 applied=747.0, repeat layoutPasses=0 | |
...
| 4 | R1 red 이후에만 잘못된 fixed point의 원천을 수정하고 높이·배치·수렴·scroll restore oracle을 닫는다 | 근본 레이아웃 수정 | (a) 높이만 보는 oracle은 빈 띠를 통과시킴 (b) stack의 38pt 자체 top inset과 `NSScrollView` 자동 32pt title-bar inset이 중복됨 (c) 실기기 154pt 차이는 별도 미해명 | `app/Sources/App/SetupWindowController.swift`, `app/Tests/AppTests/SetupWindowLayoutTests.swift`, `app/Tests/AppTests/SetupWindowRedrawTests.swift` | 1∼3에서 red 확인 | claimed | 재실행: `cd <clone> && swift test --package-path app` → 481 tests / 1 skipped / 0 failures / exit 0. 토글: `git apply -R inset-fix.patch` → 481 / 1 skipped / 11 failures / exit 1, 전부 축소 방향(accessibility-granted 2건, terminal 전이 8건, 설정 되읽기 1건). 확대 방향과 긴 문서 정지 상태는 토글에서 red가 아니다. `assertFittedPlacement`는 권한 부여 축소·권한 역전 확대·터미널 전이·bottom-edge 성장의 4개 지점에서 호출된다 (D29∼D31) | |
...
값의 흐름을 확인한 결과 `SetupWindowController.buildContent()`(406∼455)가 이 창의 유일한 `NSScrollView` 생성·documentView 배선 지점이며, `rebuildForLanguageChange()`(303∼314)는 306행에서 그 함수를 다시 호출해 새 scroll view를 `window.contentView`에 넣는다. ordinary `refresh()`와 각 알림 경로는 scroll view를 새로 만들지 않고 기존 stack의 값만 바꾼다. 따라서 `buildContent()`의 자동 inset off는 최초 생성과 언어 변경 재구성을 모두 덮지만, 각 값 변화가 `FittedContentStackView.layout()`까지 도달하는지는 아래 행별 oracle로 판정한다.

| 대상 | 판정 | 코드로 알 수 없는 이유 또는 `파일:행` |
|:--|:--|:--|
| 초기 `SetupWindowController` 생성: `buildContent()`의 guide·install feedback·test result 기본 hidden → `updateTerminalControls()` → 첫 `refresh()` → `layoutSubtreeIfNeeded()` | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:191∼226`, `406∼455`, `467∼485`, `727∼745`; 새 scroll view가 만들어질 때 자동 inset off가 적용되고 첫 측정은 hidden state·status wrapping·screen clamp와 함께 settle된다. |
| `manifest` → `chromeCard.isHidden` | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:991∼1002`; manifest 완료/미완료가 card height를 바꾸고 기존 scroll view에서 `FittedContentStackView.layout()`을 다시 dirties한다. |
| `lastRequestAt`·extension state → `extensionCard.isHidden`, `utilityRow.isHidden` | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:1004∼1015`; socket evidence가 card를 줄이고 utility row를 되살리는 양방향이며 같은 scroll view의 inset 계약을 공유한다. |
| `Settings.toolAvailability == nil` 또는 missing tools/wrapper advice → `toolsCard.isHidden`와 `toolsList` 재구성 | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:1117∼1141`; list removal/addition은 hidden 토글이 아니어도 intrinsic height를 바꾸며 buildContent의 자동 inset 설정은 유지된다. |
| `Settings.terminal`과 iTerm automation 상태 → `permissionSection.isHidden` | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:1029∼1050`; `select(terminal:)`의 iterm↔wezterm↔warp가 section을 늘리고 줄이는 양방향을 같은 scroll view에서 처리한다. |
| `PermissionChecker.isAccessibilityGranted` → `accessibilitySection.isHidden` | 공통 경로 덮음(항목 1·2·4) | `SetupWindowController.swift:1025`, `1051`; provider가 false→true/true→false를 만들고, 자동 inset off가 권한 전이 후 재측정에도 적용된다. |
| `select(terminal:)` → radio/note text → `refresh()` | 공통 경로 덮음(항목 2·4) | `SetupWindowController.swift:949∼974`; terminal card text와 permission section이 동시에 변하고 `testWindowMatchesContentAcrossEveryTerminalTransition` 및 growth placement가 양방향을 본다. |
| `windowDidBecomeKey` → `refresh()`; System Settings에서 돌아온 Accessibility 상태의 실제 관측 지점 | 공통 경로 덮음(항목 1·2·4) | `SetupWindowController.swift:235∼237`, `1348∼1354`; key 복귀가 상태 재조회 경계이며, 동일한 buildContent scroll 설정과 fixed-point oracle을 사용한다. |
| `.terminalCheckoutRequestHandled` → extension evidence/card, `.terminalCheckoutToolsChecked` → tools card | 공통 경로 덮음(항목 4) | observer는 `SetupWindowController.swift:216∼222`, 발생원은 `Settings.swift:67∼71`, `96∼107`; 비동기 hidden 변화도 기존 scroll view의 자동 inset off와 layout pass를 공유한다. |
| `languageChanged` → Settings notification·explicit `refresh()` → `rebuildForLanguageChange()` | 공통 경로 덮음·restore 보존(항목 4) | `SetupWindowController.swift:303∼314`, `406∼455`; 306행이 buildContent를 다시 호출하므로 새 scroll view에도 자동 inset off가 적용되고, 305행 capture와 313행 restore의 비뒤집힌 계약은 바꾸지 않는다. |
| `registerManifest`, `installInChrome`, `reshowInstall` → guide/feedback hidden 변경 및 `refresh()` | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:1235∼1264`, `1320∼1328`; 직접 label visibility를 바꾼 직후 refresh가 다시 상태를 덮고 같은 scroll view에서 intrinsic size를 재측정한다. |
| `baseDirectoryEdited` → invalid status label text 또는 valid `refresh()` | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:1278∼1300`; invalid path는 refresh 없이 wrapping label만 바꾸므로 Fitted stack이 text intrinsic change와 inset 없는 clip을 함께 측정한다. |
| `requestPermission` completion → permission status·`refresh()` | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:1330∼1341`; iTerm callback은 Accessibility와 다른 비동기 경로지만 permissionSection의 크기 변화를 같은 scroll setup으로 처리한다. |
| `requestAccessibility()` 즉시 `refresh()`와 key 복귀 refresh | 공통 경로 덮음(항목 1·2·4) | `SetupWindowController.swift:1348∼1354`; prompt 직후 old state일 수 있고 key 복귀에서 실제 전이가 오며, 두 refresh 모두 동일한 scroll view 설정을 유지한다. |
| `testTerminal` 시작/완료 → `testResultLabel.isHidden`·status text·`refresh()` | 공통 경로 덮음(항목 4) | `SetupWindowController.swift:1360∼1382`; async 완료가 label visibility와 text를 바꾸고 권한 경로와 같은 intrinsic-size/inset oracle을 공유한다. |
| `windowWillClose` → guide/feedback/test result hidden | 안전·다음 생성에서 덮음 | `SetupWindowController.swift:240∼247`; 닫히는 창의 크기 oracle은 아니지만 다음 open은 buildContent를 다시 타므로 자동 inset off가 초기 hidden 상태에 재적용된다. |
| `visibleFrameOverride` 대입 → stack layout 예약 | 공통 경로 덮음·setter 보존 | `SetupWindowController.swift:53∼55`; `SetupWindowLayoutTests.swift:64∼74`, `180∼201`; setter가 layout을 예약하는 계약은 유지되고 inset off는 display override와 독립적이다. |
| `visibleFrameOverride ?? window.screen ?? NSScreen.main` → clamp height와 `moveInside` | 원인 아님 확인·미검증 분기(항목 3·5; 항목 4 제외) | `SetupWindowController.swift:61∼62`; 자동 테스트는 override를 사용해 `window.screen`·`NSScreen.main`을 지나지 않는다. D24 실기기에서 `NSScreen.main`은 세로 `visibleFrame.height=2530`, 창은 가로 `visibleFrame.height=1440` 위에 있었지만 콘텐츠 최대 794pt라 어느 쪽에도 clamp되지 않았다. 실제 화면 선택 분기는 여전히 자동 테스트에서 미검증이다. |
| `settle(_:)` 고정점 반복 → 실제 `layout()` 재진입 고정점 | 공통 경로 덮음·계측 유지(항목 2·4) | `SetupWindowLayoutTests.swift:99∼118`; `SetupWindowRedrawTests.swift:62∼75`; pass 수·fittingSize·target·window/document frame·documentVisibleRect·scroll origin을 N회까지 기록하고, 이번 승격에서는 `layoutProbeForTesting`를 유지한다. |
| `8fccab5`의 visibility→settle→measure·clamp·scroll 계약 → 권한 text/hidden 전이 | 공통 경로 덮음·선행 공백 보완(항목 3·4) | `git show 8fccab5`; 이번 입력은 `select(terminal:)`가 아니라 `windowDidBecomeKey` 뒤 `refresh()`이며, buildContent의 중복 자동 inset을 제거해 placement oracle까지 닫는다. |
| `lastRequestedSize` early return과 `FittedContentStackView.layout()`의 measurement boundary | 공통 경로 덮음·크기 축 유지(항목 2·4) | `SetupWindowController.swift:57∼75`; D20에서 fitting/target/applied와 pass 수는 원인이 아님으로 기각됐고, 이번 변경은 early return이나 fitting 측정 경계를 바꾸지 않는다. |
| `capturePlace`/`restore`의 비뒤집힌 document top 및 hidden anchor fallback | 공통 경로 덮음·보존 필수(항목 3·4) | `SetupWindowController.swift:303∼385`; `SetupWindowRedrawTests.swift:224∼338`; flipped나 origin clamp를 도입하지 않고 buildContent 재구성에도 자동 inset off만 재적용하므로 anchor·focus·selection 복원을 그대로 검증한다. |
