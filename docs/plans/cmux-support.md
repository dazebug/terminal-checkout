...
| `App/SetupWindowController.swift:871-873,1266,1414,1421` (`isITermInstalled`·`isWezTermInstalled`·`isWarpInstalled` 호출처; cmux 라디오·상태 카드 누락) | 예 — 설치 표시·상태 UI의 cmux 누락은 항목 14 소관 | 눈으로 | 구멍(항목 14) |
...
- 부류 3(감지·상태 프로브): 항목 12는 `terminalForInstalledTerminals` 순수 우선순위 함수와 Settings 연결, 항목 13은 `CmuxSocketStatus` 분류·라이브 `cmux ping`·5로케일 label로 claimed했다. TDD 테스트는 자동 감지 4개, 상태 분류 5개, 상태 label 5상태 × 5로케일이다. 게이트는 드라이버가 실행한다.
