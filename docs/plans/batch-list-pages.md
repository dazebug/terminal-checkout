...
- `app/Sources/Core/CommandRenderer.swift:32-34` — 현재 `allowedVariables`에는 `repo`, `branch`, `base`, `main`, `branch_underbar`, `number`, `owner`만 있어 스펙이 요구하는 `{pr}`·`{issue}` list preset은 구현 전에 whitelist를 열어야 한다. 실행·검증 semantics는 이 두 이름 추가로 바꾸지 않는다.
...
| cmux 패널 (점검 블록 `cmux:` 신호가 켜졌을 때만, 아니면 N/A) — `cmux markdown open <작업 트리 계획 파일 절대경로>` → pane id. 계획 파일 첫 승격 전에 채운다 | N/A; 이 R0에는 `cmux:` 점검 신호가 없다 |
...
|:--|:--|:--|:--|:--|:--|:--|:--|:--|
| 0 | 앱 renderer whitelist에 `pr`·`issue`를 추가하고 기존 render test 파일에 red→green 테스트를 추가한다 | app variable contract | (a) `{pr}`/`{issue}` 렌더가 `unknownVariable`로 거부됨(red 재현: `renderCommand(template: "echo {pr}", variables: ["pr": "7"])` → 현 트리 throw) | `app/Sources/Core/CommandRenderer.swift`, `app/Tests/CoreTests/` (기존 render test 파일) | — | todo | `nl -ba app/Sources/Core/CommandRenderer.swift | sed -n '32,34p'` → 두 이름이 현재 whitelist에 없음; 기존 render test 파일에 같은 호출을 먼저 red로 추가한 뒤 whitelist와 함께 green 확인 | Swift gate는 driver 실행; 실행·검증 semantics는 그대로 |
...
| `README.md` 변수 표·Development·permission 문구, `CLAUDE.md`, `docs/new-terminal-checklist.md` | 구멍(항목 7) | `README.md:119-175,229-247`, `CLAUDE.md`의 shipped preset 수, checklist batch fan-out 행에 list selection·변수·결과 UI가 없음 |
| `app/Sources/Core/CommandRenderer.swift`·`app/Sources/Core/Request.swift`·`app/Sources/App/HostServer.swift` 및 app Swift tests | 구멍(항목 0)·안전(그 밖의 읽기 전용 계약) | `CommandRenderer.swift:32-34`의 whitelist에 `{pr}`·`{issue}`가 없어 항목 0에서만 두 이름을 추가한다. PR #73 `1f3ef87`; `Request.swift:81-350`와 `HostServer.swift:184-279`의 batch 실행 계약은 그대로 둔다 |
