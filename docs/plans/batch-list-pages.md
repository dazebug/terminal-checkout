...
|:--|:--|:--|
| `extension/defaults.js`의 `pageTypeOf`·`REPO_TABS`·`RESERVED_OWNERS` | 완료(항목 1)·구멍(항목 2) | `defaults.js:209-229` — `pageTypeOf`만 `RESERVED_OWNERS` guard와 `REPO_TABS` fallback을 소비한다. exact `/owner/repo/pulls`·`/issues`와 trailing slash은 generic tab 판정보다 먼저 `pr-list`·`issue-list`가 되고, `/pulls/extra` 같은 nested path는 기존 `repo` fallback을 유지한다. 두 상수의 범위와 BUTTON_KINDS/SETTINGS_KEYS/preset 변경은 항목 2에서 다룬다. 다른 runtime consumer는 발견되지 않았다 |
...
| `extension/content.js`의 `pageTargetOfUrl`·`tryInsertButton`·`tryInsertRepoButtons`·`onUrlChange`·`removeInsertedButtons`·observer/polling 경로 | 부분 완료(항목 1)·구멍(항목 3, 4, 5) | `content.js:92,319-365,372-442` — click target은 shared URL parser에서 읽고, list kind도 `tryInsertButton`의 기존 unconditional `tryInsertRepoButtons`를 통해 repository header button을 계속 받으며 PR/issue detail branch만 건너뛴다. `onUrlChange`의 `sameTarget`은 list↔detail kind 변화에서 `removeInsertedButtons`를 호출하고, query-only pagination/filter에서는 같은 target로 보아 기존 button을 유지한다. `removeInsertedButtons`는 현재 삽입된 공통 button class만 제거하며, list row DOM과 fallback selection 재주입은 항목 3에서 정한다. observer와 1s polling은 기존 header trigger를 유지하고 list toolbar/row selector는 후속 항목으로 남긴다 |
| `extension/background.js`의 `parseGitHubUrl`·icon·`RUN_BY_KIND`·`onMessage`·`assertSamePage`·`isPageTarget`·`isRepoPage` | 부분 완료(항목 1)·구멍(항목 4) | `background.js:15-16,179-180,202-213,341-351,368-409,415-433` — `parseGitHubUrl`, `assertSamePage`, live current gate와 `onMessage`의 `isPageTarget`은 shared URL/target을 사용해 list kind와 null number를 허용하되 kind 불일치를 막는다. icon은 `isRepoPage` DOM 검증 후 `iconRunKind`에서 list를 기존 `repo` runner로 폴백하고, `RUN_BY_KIND`·`ACTION_KIND`에는 list runner/action을 추가하지 않는다. `executeRepoCommand`가 list target을 받는 것은 재분류 전 리스트에서 실행되던 repository icon 동작을 보존하기 위한 것이며, selection batch message/inner response는 항목 4의 몫이다 |
...
| `tests/buttons.test.js`·root `node --test` | 완료(항목 1)·구멍(항목 6) | `buttons.test.js`의 `pageTypeOf: PR, issue, repository, and list pages`가 exact list/trailing slash/nested path 및 `RESERVED_OWNERS` 경계를 고정하고, `pageTargetOf: what a request is built from, read off one pathname`가 list target의 null number를 고정한다. `sameTarget: a navigation between two PRs is not the same target`가 list↔detail·PR-list↔issue-list kind 차이를 고정하며, 기존 `pageTargetOfUrl`·`requestIsCoherent`·`isPageTarget` gate 테스트가 URL/target 소비 흐름을 덮는다. red `node --test` exit 1, `tests 222`, `pass 219`, `fail 3`에서 actual `repo`/`true`를 확인했고 green은 exit 0, `tests 222`, `pass 222`, `fail 0`이다. 실제 list DOM harness는 만들지 않고 항목 3·6 및 hands-on checklist에서 다룬다 |
