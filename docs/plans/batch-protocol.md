...
- Current: R4 class 4 verification corpus and terminal checklist are verified; R4a's batch wire-key correction is claimed, and classes 1–3 are cleared and committed.
- Recent verdict: driver decisions D3–D12 are recorded below; the direct legacy and batch Core probes passed, while the Swift gate remains driver-only in this sandbox because this environment cannot build the manifest.
...
| 4 | Verification corpus and terminal checklist | verification / documentation | (a) `app/e2e.sh` exercises only legacy failure cases; (b) `docs/new-terminal-checklist.md` has no batch launch, result, cap, or parallel-delivery path; (c) Warp fan-out visibility, the cross-terminal batch oracle, and the driver cmux pane check are not documented. | `app/e2e.sh`; `docs/new-terminal-checklist.md` | 1, 2, 3 | verified | `app/e2e.sh:31-162` adds four batch failure cases and validates decoded `success`, `error`, and `items` through Python exit status; `docs/new-terminal-checklist.md:87-93` adds five hands-on batch checks for sequential N-tab fan-out, per-item results and arrival-anchored timelines, one Warp warning, the four-permit/fifth-waits bound, and cmux background completion; rerun with `bash app/e2e.sh` and inspect with `rg -n 'batch|fan|concurr|Warp|cmux' app/e2e.sh docs/new-terminal-checklist.md`. verified — driver ran bash app/build.sh && bash app/e2e.sh in the clone: exit 0, all 13 cases passed (9 legacy + 4 batch). | — |
| 4a | Batch template wire-key labels in the shared resolver | protocol text | `resolveRequestItem` calls `rejectNUL(in: command, what: "command_template")` for batch requests, so a batch NUL failure names a key the request does not carry. | `app/Sources/Core/Request.swift`; `app/Tests/CoreTests/BatchProtocolTests.swift` | 1 | claimed | `BatchRequestTests.testBatchCommandNULRejectionNamesTheCommandWireKey` expected `command must not contain NUL`; the pre-fix Core probe returned `command_template must not contain NUL` and the assertion exited 1; the caller now supplies `command` for batch and `command_template` for legacy, with the legacy corpus unchanged; rerun with `cd app && swift test --filter BatchRequestTests`. | — |
