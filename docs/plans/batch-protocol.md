...
- Current: R3b class 2 gate correction and request-arrival timeline review are in progress; classes 1 and 3 are cleared and committed, class 4 remains untouched.
...
| 2 | Core batch fan-out orchestration and thin HostServer response path | execution / response | (a) Core currently models one resolved request and one run callback; (b) HostServer invokes the request handler and terminal runner once per frame; (c) there is no all-item validation preflight, ordered item result, or validated-launch failure continuation rule; (d) there is no batch-level Warp typed-delivery warning or per-item timeline path. | `app/Sources/Core/Request.swift`; `app/Sources/Core/Logging.swift`; `app/Sources/App/HostServer.swift`; `app/Tests/CoreTests/BatchProtocolTests.swift`; `app/Tests/AppTests/HostProtocolTests.swift` | 1, 3 | verified | `app/Sources/Core/Request.swift:237-329`; `app/Sources/Core/Logging.swift:26-59`; `app/Sources/App/HostServer.swift:171-250`; `app/Tests/AppTests/HostProtocolTests.swift:164-610`. No-change proof: launch remains serialized via `execQueue.sync` and response still writes `JSONSerialization` bytes directly in `app/Sources/App/HostServer.swift:180-250`. Red evidence: 신규 6 테스트는 f869ddb에는 없는 주입점(runInTerminalFactory 등)을 사용하므로 구 트리에서 컴파일 불가 = 구조적 red. Test set: 7 HostProtocolTests cases, including `testServeSecondItemTimelineTotalIncludesFirstLaunchTime`; rerun with `cd app && swift test --filter HostProtocolTests`. verified — driver reran cd app && swift test in the clone: exit 0, Executed 580 tests, 0 failures (baseline 573 + 7 new); first driver gate caught six missing try sites (exit 1), fixed in R3b. | — |
