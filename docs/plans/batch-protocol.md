...
| 3 | Bounded parallel Claude delivery | concurrency / lifecycle | (a) every current delivery is dispatched directly to a global utility queue; (b) `ClaudeDelivery.Admission` tracks lifecycle and helper cleanup but imposes no active-delivery cap; (c) batch fan-out would therefore run unbounded `ps`/`stty`/screen polling; (d) there is no scheduler test proving a global cap of 4, whole-call coverage, release, or shutdown behavior. | `app/Sources/Core/ClaudeInjector.swift`; `app/Tests/CoreTests/BatchDeliveryTests.swift` (new) | — | verified | 3 tests in `app/Tests/CoreTests/BatchDeliveryTests.swift`: `testDeliveryPermitAllowsAtLeastTwoConcurrentCalls`, `testDeliveryPermitCapsConcurrentCallsToAtMostFour`, `testDeliveryPermitReleasesOnFailureSoNextCallCanRun`; rerun with `HOME=/private/tmp swiftc -typecheck -module-name Core -module-cache-path /private/tmp/clcore-cache app/Sources/Core/*.swift` and `cd app && swift test --filter BatchDeliveryTests`; red baseline (before wrapper) was a direct probe compile failure: `cannot find 'withClaudeDeliveryPermit' in scope`; verified — driver reran cd app && swift test in the clone: exit 0, Executed 573 tests, 0 failures (baseline 570 + 3 new). | — |
