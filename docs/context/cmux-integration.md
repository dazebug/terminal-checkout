...
If it never appears, the command has already started; only the claude input is given up, with a log line.
...
## Split order halves the active pane; balanced splits and equalization control N=4 geometry

**Type:** decision
**Status:** active
**Evidence:** confirmed in cmux 0.64.22 (102) [ddd4a01bc]; issue #68 item 5 and item 7
**Source:** Issue #68 item 5 and item 7
...
**Rejected alternative — `N ≤ 3`.** The earlier proposal was based on the unverified assumption that four panes lay out unreadably: the measurements show balanced 2×2 geometry with 154 columns per pane, while the linear shape has 38-column panes, but visual readability at 38 columns was not judged.
...
| `surface.create` | `surface.create` with optional `type` / `workspace_id` / `pane_id` | returns `pane_id`, `pane_ref`, `surface_id`, `surface_ref`, `type`, `window_id`, `window_ref`, `workspace_id`, and `workspace_ref`; `index_in_pane` is observed only in a later `surface.list`; unknown `pane_id` returns `not_found` | same floor; missing params are valid defaults | creates a tab in an existing pane, does not create a new pane | use when caller needs another surface in one pane |
...
| `surface.send_text` | `surface.send_text` with `surface_id` and `text` | response includes `queued` plus IDs; queued indicates warm-up state, not loss | not measured for this issue | with no tty yet, `queued:true` is expected and flushes on warm-up | do not infer safe retries; the existing no-retype contract governs command delivery |
