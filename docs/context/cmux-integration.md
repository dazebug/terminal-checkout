...
**Type:** decision
**Status:** active
**Evidence:** confirmed; issue #68 items 5, 7–13 and driver measurements from 2026-09-01
**Source:** Issue #68 measurement comment and issue #69 driver measurements, 2026-09-01; https://github.com/dazebug/terminal-checkout/issues/68#issuecomment-5487928294
...
Grouped placement prefers a layout-built `workspace.create` whenever it can create a workspace: always-new pane placement, fixed-name not-found pane placement, and create-based tab or workspace-per-item routes. A found fixed-name workspace cannot receive a layout after creation, so that path starts at the first surface of `pane.index` 0 and applies a target-addressed balanced `surface.split` sequence. N>8 pane requests use the U1 tab-per-item fallback rather than an unmeasured pane shape.

There are two item-to-surface sources of truth. Layout-created leaves are enumerated through `pane.list` and one workspace-scoped `surface.list` call in depth-first `pane.index` order; the returned surfaces are grouped by each item's owning `pane_id` and then ordered by `index_in_pane`. cmux ignores a `pane_id` parameter on `surface.list`, so the app omits that parameter instead of treating a repeated full-workspace response as a pane filter. Found-split items use the depth-first order obtained by collecting each split response's `surface_id`. Each grouped plan carries one UUID `operation_id` for the batch and one UUID per item for workspace-per-item creates; cmux rejects a non-UUID `operation_id` (driver measurement, 2026-09-01). The guarded-send loop consumes a response-deadline seam before each item, so a deadline-cut item records the not-launched failure while already-created surfaces remain.
...
**Consequence, accepted:** placement paths are observable through the existing per-item timeline and `checkoutLog`, and guarded commands fail closed at the response deadline without rollback. The found-split order for N∈{2,6,7}, high-volume N=25 tab behavior, and the scope of `workspace.list` across multiple windows remain unmeasured; the manual checklist records these as boundaries rather than extending the measured claim.
...
| pane enumeration | `pane.list` with `workspace_id` | returns `panes[]`; placement reads each item's `id` and `index` | no placement retry | the workspace target is explicit | order panes by the measured `index` sequence before surface grouping |
