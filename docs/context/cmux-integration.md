...
A third transport form is measured from the server as `Error: Path exists at <path> but is not a Unix socket`, but it is not classified today, so that path still stays fail-closed in the contract.
...
## Split order halves the active pane; balanced splits or `equalize_splits` keep N=4 readable
...
The measured `layout` node shape is a branch `{"direction":"horizontal"|"vertical","split":<0..1>,"children":[<node>,<node>]}` or a leaf `{"pane":{"surfaces":[{"type":"terminal","command":"…"}]}}`; `split` is optional and its omission produced an even division, two 1160 px panes in a 2320 px window. A bare leaf is a valid whole layout and produced one pane whose command ran. Every branch has exactly two children: a flat four-child node was rejected with `Error: invalid_params: Invalid layout: …`. Nesting composes: a three-pane tree with `split:0.34` and a right child split of `0.5` produced 788.8 / 765.6 / 765.6 px panes, and the measured 2×2 produced four equal 1160 × 691 px panes.
