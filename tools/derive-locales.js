#!/usr/bin/env node
// Writes `extension/_locales/<directory>/messages.json` from `extension/_i18n/<tag>.js`.
//
//     node tools/derive-locales.js          # rewrite the five catalogues
//     node tools/derive-locales.js --check   # exit 1 if what is committed is not what this derives
//
// **A derivation and not a copy.** Two stores holding the same 122 values can be edited into
// agreement — a wrong edit made in both passes any equality check anyone would write — so the second
// store is produced by a program while the first is still the only one anybody edits. This runs
// during A2, and A3 is the commit that makes `_locales` canonical; after that the generator is
// retired and only the checker survives (D167), which is why `--check` exists here from the start.
//
...
const main = () => {
  ...
  for (const tag of context.TC_I18N_LOCALES) {
    ...
    const derived = serialize(messages);
    if (check) {
      if (fs.readFileSync(file, 'utf8') !== derived) {
        differences += 1;
    ...
    process.stdout.write(`_locales/${directory}/messages.json <- _i18n/${tag}.js (${Object.keys(messages).length} names)\n`);
  }
  if (check && differences > 0) process.exit(1);
  if (check) process.stdout.write(`all ${context.TC_I18N_LOCALES.length} catalogues match what _i18n derives\n`);
};
